<template>
  ...
  <div class="bg-white border rounded-lg p-4 m-4">
    <div
      ...
      :key="job.job_id"
      class="border-2 border-violet-500 rounded-lg p-4 m-4"
    >
      ...
      <div
        v-if="showDropdown(job.job_id)"
        class="mt-2 bg-white border-2 border-violet-500 border-gray-300 p-4 rounded-lg shadow-lg"
      >
        <div class="mb-2">
          ...
          <p class="text-sm text-gray-600">
            <strong>Ended At:</strong> {{ job.ended_at }}
          </p>
          <p class="text-sm text-gray-600">
            <strong>Time Taken:</strong> {{ job.time_taken }} seconds
          </p>
          <br>
          <pre class="text-sm text-gray-600 p-2 bg-gray-100 rounded-md">{{
            job.arguments
          }}</pre>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      const apiUrl =
        '/api/method/flashdesk.api.rq_stuff.get_new_completed_queues2'

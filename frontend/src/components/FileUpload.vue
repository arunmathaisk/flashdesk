<template>
    <div class="w-full" @dragover="prevent" @drop="dropFile">
      <label class="block font-medium leading-6 text-gray-900 py-2">{{ label }}</label> 
      <div v-if="!uploaded_files"
        class="flex justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none cursor-pointer hover:border-gray-400 focus:outline-none">
        <span  class="flex items-center space-x-2">
            <svg @click="removeFile" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-600" fill="none" viewBox="0 0 24 24"
        ...
        <input required @change="dropFile" type="file" ref="file" name="file_upload" class="hidden" multiple>
      </div>
      <div v-else
        ...
        <div class="text-gray-600 font-medium flex items-center justify-between">{{ uploaded_files.name}}
            <div @click="removeFile">
              <svg class="font-medium w-5 h-5" viewBox="0 0 21 21" xmlns="http://www.w3.org/2000/svg">
...
<path d="m4 1v4h-4" transform="matrix(1 0 0 -1 0 6)"/>
        ...
        <div class="mb-5 h-2 rounded-full bg-gray-200 w-full">
            <div class="h-2 rounded-full bg-orange-500" :style="'width:'+upload_progress+'%'"></div>
        </div>
        <br>
...
export default {
    ...
    data(){
        return {
          ...
          total_chunks:0,
          chunksize:1000000,
          file_upload_endpoint:"/api/method/flashdesk.api.fileupload.files.file_upload",
          upload_progress:0
        }
    ...
    methods:{
        async submitFile(){
          ...
          let currentchunk = 0;
          for(let start = 0.0;start < this.uploaded_files.size;start += this.chunksize){
            const chunk = this.uploaded_files.slice(start, start + this.chunksize + 1)

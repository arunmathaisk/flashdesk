      ...
      <div v-for="detail in form_details" class="p-4">
        ...
        <div v-else-if="detail.type == 'select'">
          <label
            ...
            :class="detail.defaultRequired ? 'text-red-500' : 'text-gray-900'"
            >{{ detail.label }}</label
          ><br />
          <select
            ...
            :name="detail.name"
            :class="
              detail.defaultRequired ? 'border-red-300' : 'border-black-300'
            "
          >
        ...
        <div v-else>
          ...
          <div class="mt-2">
            <input
              ...
              v-model="detail.value"
              :type="detail.type"
              class="block w-full font-medium rounded-md border-1 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
              :class="
                detail.defaultRequired ? 'border-red-300' : 'border-black-300'
              "
            />

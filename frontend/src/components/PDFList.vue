<template>
  ...
  <div class="p-4 m-2">
    ...
    <div v-if="pdflist.length != 0">
      <li v-for="x in pdflist" class="flex items-center justify-between">
        {{ x }}
        <svg
          @click="delete_pdf(x)"
          height="10px"
    ...
    <div v-else>No files for the Chatbot here yet</div>
  </div>
</template>
  ...
  methods: {
    ...
    async start_processing() {
      const process_endpoint =
        '/api/method/flashdesk.api.ai.process_pdfs.start_processing'

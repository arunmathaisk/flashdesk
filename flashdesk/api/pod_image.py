...
@frappe.whitelist()
def get_all_available_pod_images(search_term):
	return get_all_filesystem_docker_images(search_term)
...
def create_image_from_file():
	try:
		...
		pod_image = frappe.get_all(
			"Pod Image", fields=["image_file", "image_name"], filters={"name": pod_id}
		)
		if not pod_image:
		...
		return {
			"status": "success",
			"message": "The image extraction process has started in the background. Please check Events for more informstion.",
		}
